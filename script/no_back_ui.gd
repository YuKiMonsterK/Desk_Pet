...
func _process(_delta):
	...
	if character_p != 0 and (int(game_window.position.x) - int(character_p) > 5 
	or int(game_window.position.x) - int(character_p) < -5):
		if game_window.position.x > character_p:
			direction = -1
		else:
			direction = 1
		game_window.velocity.x = direction * 100
		SignalManager.emit_signal("character_walk",direction)
		game_window.move_and_slide()
	#如果相距小於20就判斷完成移動
	elif character_p != 0:
		character_p = 0
		SignalManager.emit_signal("character_stop")
